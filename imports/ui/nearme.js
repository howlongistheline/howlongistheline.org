...
function Index({ history }) {
  ...
  function checkClientLocation() {
    if (clientLocation.location != undefined) {
      if (
        (new Date().getTime() -
          new Date(clientLocation.location.time).getTime()) /
          1000 <
        300
      ) {
      ...
      <Card key={location._id} style={{ backgroundColor: 'white' }}>
        ...
        <ListItem modifier="nodivider">
          ...
          <div className="right">
            <Button
              onClick={() => {
                history.push(`/stocks?id=${location._id}`);
              }}
            >
              Stocks Status
            </Button>
        ...
        <ListItem modifier="nodivider">
          <div className="center">
            If you are at this store right now, drag the slider to update the
            line numbers, or confirm the existing numbers.
          </div>
        </ListItem>
        <ListItem modifier="nodivider">
          <div className="center">
            <Slider
              defaultValue={
                parseInt(location.line) ? parseInt(location.line) : 0
              }
              min={0}
              max={50}
