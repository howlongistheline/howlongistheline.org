...
function EditLine({ history, details }) {
  ...
  const [lineSize, setLineSize] = useState(
    parseInt(new URLSearchParams(window.location.search).get('lineSize')),
  );
  ...
  function renderCard(location) {
    ...
    return (
      <Card key={location._id} style={{ backgroundColor: 'white' }}>
        ...
        <ListItem modifier="nodivider">
          <div className="center">
            Press Submit below to confirm that you are at the store, and there{' '}
            {lineSize === 1 ? 'is' : 'are'} {lineSize || 0}{' '}
            {lineSize === 1 ? 'person' : 'people'} in line right now.
          </div>
        </ListItem>
      </Card>
    );
  ...
  function submit() {
    ...
    function error(err) {
      toast(
        "Can't get current location, please turn on browser's geolocation function or try a different browser",
      );
      setLoading(false);
  ...
  return (
    <MainLayout>
      <div style={{ marginBottom: 55 }}>
        <Card>
          Are you really at this location? It appears that either you are not at
          the store, or we have the wrong coordinates for the store.
        </Card>
        <Card>
          If you are not at this store right now: ask your friends to update
          this store for you whenever they go.
        </Card>
        <Card
          onClick={() => {
            console.log(123);
        ...
        >
          If you are at this store right now, please submit to reset the
          location. If you are intentionally providing misleading information
          your IP address, device fingerprint, and physical location will be
          published.
        </Card>
        {renderCard(details)}
      </div>
